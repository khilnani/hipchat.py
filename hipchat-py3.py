...
def display_unread(items):
    ...
    if 'iP' in machine:
        if not appex.is_running_extension() and not appex.is_widget():
            display_unread_ios(items)
        elif appex.is_widget():
            lines = len(items) + 4
            appex.set_widget_height(lines * 20)
    else:
        show_details = check_show_details()
        if show_details == True:
            display_unread_desktop(items)
        elif show_details == None:
            show_details_sel = eval(input('Show details? (y/n): '))
...
def main():
    ...
    if not items:
        ...
        if timeleft:
            print('Please wait %s to honor api limits.' % timeleft)
            sys.exit(1)

        if not check_access_token(api_url, access_token):
            get_new_access_token(api_url, base_url, useremail)
            print('Configuratiom updated with access token. Start over please.')
            sys.exit(1)
